...
type CurrentWeatherDataRetriever interface {
	GetByCityAndCountryCode(city, country string) (Weather, error)
	GetByGeoCoordinates(latitude,longitude float32)(Weather,error)
}
...
func GetMockData() io.Reader{
	...
	"id":3117735,"name":"Madrid","cod":200}`
...
func (d *CurrentWeatherData)GetByGeoCoordinates(lat,lon float32)(weather *Weather,err error){
	return d.doRequest(fmt.Sprintf("http://api.openweathermap.org/data/2.5/weatherq=%f,%f&APPID=%s", lat, lon, d.APIKey))
}

func (d *CurrentWeatherData)GetByCityAndCountryCode(city, countryCode string) (weather *Weather,err error){
	return d.doRequest(fmt.Sprintf("http://api.openweathermap.org/data/2.5/weather? lat=%s & lon=%s & APPID=%s", city, countryCode, d.APIKey))
}

func (d *CurrentWeatherData)doRequest(uri string)(weather *Weather,err error){
	client := &http.Client{}
